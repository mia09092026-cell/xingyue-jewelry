...
function fillRequiredInquiryFields() {
  ...
  fireEvent.change(screen.getByLabelText("Custom Requirement"), { target: { value: "Private label packaging" } });
...
describe("Contact inquiry page form", () => {
  ...
  it("renders the original core inquiry fields and consent control", () => {
    render(<ContactPage />);
    for (const label of ["Name", "Company", "Email", "WhatsApp / Phone", "Country", "Product Interest", "Target Quantity or Range", "Custom Requirement", "Message"]) {
      expect(screen.getByLabelText(label)).toBeRequired();
    }
    for (const removedLabel of ["Business Type", "Target Market", "Reference Image / Design", "Material", "Stone", "Expected Timing"]) {
